...
function About() {
  ...
  return (
    <div className="min-h-full flex flex-col">
      ...
      <div
        className="flex-1 bg-cover bg-center bg-no-repeat relative flex flex-col justify-between items-center pt-8 px-4 md:px-10"
      ...
      >
        ...
        <div className="animate-fade-in border-2 border-violet-400 rounded-xl m-8 p-6 min-h-[400px] bg-violet-300/70 flex flex-col md:flex-row items-center justify-center gap-10 z-10 w-full max-w-6xl">
          ...
          <div
            className="w-48 h-48 sm:w-64 sm:h-64 lg:w-[400px] lg:h-[400px] justify-items-center content-center shrink-0 z-10 overflow-hidden rounded-full bg-cover bg-center bg-no-repeat shadow-2xl"
          ...
          <div>
            <h2 className="text-2xl sm:text-4xl lg:text-5xl mb-8 font-mono text-center shrink-0 z-10">
              {miembroSeleccionado.nombre}
            </h2>
            <p className="text-gray-700 text-center max-w-lg text-base sm:text-lg lg:text-xl mt-8">
              {miembroSeleccionado.description}
            </p>
            ...
            <div className="justify-self-center">
              {miembroSeleccionado.id !== 0 && (
                <button
                  ...
                  className="flex justify-center flex-wrap gap-5 px-2 py-2 mt-7 bg-violet-700 cursor-pointer text-white font-semibold rounded-lg shadow-md hover:bg-blue-500 hover:scale-105 active:scale-95 transition-all duration-300"
                >
              ...
              {miembroSeleccionado.id !== 0 && (
                <button
                  ...
                  className="px-2 py-2 mt-7 mx-7 cursor-pointer bg-violet-700 text-white font-semibold rounded-lg shadow-md hover:bg-black hover:scale-105 active:scale-95 transition-all duration-300"
                >
        ...
        {/* Galería — efecto difuminación con mask inline para Tailwind v4 */}
          ...
          {miembros.map((m, index) => (
            <img
              ...
              className={`brightness-50 hover:brightness-90
                ...
                opacity-90 hover:scale-110 transition-all duration-300
                hover:opacity-100 hover:z-50 relative
                ${index !== 0 ? '-ml-10' : ''}`}
              onMouseEnter={e => {
                e.currentTarget.style.maskImage = 'linear-gradient(to bottom, black 90%, transparent)';
                e.currentTarget.style.WebkitMaskImage = 'linear-gradient(to bottom, black 90%, transparent)';
              }}
              onMouseLeave={e => {
                e.currentTarget.style.maskImage = 'linear-gradient(to bottom, black 60%, transparent)';
                e.currentTarget.style.WebkitMaskImage = 'linear-gradient(to bottom, black 60%, transparent)';
              }}
            />
          ))}
        </div>
